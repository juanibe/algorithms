...
/*
    ...
    Space O(1)
*/
function averagePair(arr, avNum) {
    let left = 0;
    let right = arr.length - 1;

    while (left < right) {
        let avPairValue = (arr[left] + arr[right]) / 2;

        if (avPairValue === avNum) {
            return true;
        } else if (avPairValue < avNum) {
            left++;
        } else {
            right--;
        }
...
/*
    ...
    in the first string appear somewhere in the second string withour their order changing.
*/
